...
def parse_args():
    ...
    parser.add_argument('--stop_epochs', type=int, default=5,
...
def random_sampling_index(data_point):
    random.seed(0)
    data_index = random.sample(range(data_point), k=data_point)
    #data_index = random.randint(0, data_point)
    return data_index
...
if __name__ == '__main__':
    ...
    number_of_sampling = int(data_x.shape[0] * 0.1)
    random_sampling_index = random_sampling_index(number_of_sampling)
    data_x = data_x[random_sampling_index,:]
    data_y = data_y[random_sampling_index]
    data_id = data_id[random_sampling_index]
    #print("The dimension of the training data :{}*{}".format(data_x_random.shape[0], data_x_random.shape[1]))
    #print(data_x_random.shape)
    ...
    if train:
        ...
        print("inlinear", len(inlier_parray))
        print("outlier", len(outlier_parray))
       
        np.savetxt('inliner.csv',inlier_parray,delimiter=',')
        np.savetxt('outliner.csv',outlier_parray,delimiter=',')
